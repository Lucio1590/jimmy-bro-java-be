...
public class UserController {
        ...
        @Operation(summary = "Upload profile image", description = "Upload a profile image for the user. Image is stored in Cloudinary.", responses = {
                        @ApiResponse(responseCode = "200", description = "Image uploaded successfully"),
                        @ApiResponse(responseCode = "400", description = "Invalid file"),
                        @ApiResponse(responseCode = "403", description = "Cannot update another user's image"),
                        @ApiResponse(responseCode = "404", description = "User not found")
        })
        public ResponseEntity<UserResponse> uploadProfileImage(
                        @Parameter(description = "User ID") @PathVariable UUID id,
                        @Parameter(description = "Image file (JPEG, PNG, GIF, WebP - max 5MB)") @RequestParam("file") MultipartFile file,
                        @AuthenticationPrincipal User currentUser) {
                return ResponseEntity.ok(userService.uploadProfileImage(id, file, currentUser.getId()));
        }
        ...
        @Operation(summary = "Delete profile image", description = "Remove the profile image for a user from Cloudinary.", responses = {
                        @ApiResponse(responseCode = "200", description = "Image deleted successfully"),
                        @ApiResponse(responseCode = "400", description = "User has no profile image"),
                        @ApiResponse(responseCode = "403", description = "Cannot delete another user's image"),
                        @ApiResponse(responseCode = "404", description = "User not found")
        })
        public ResponseEntity<UserResponse> deleteProfileImage(
                        @Parameter(description = "User ID") @PathVariable UUID id,
                        @AuthenticationPrincipal User currentUser) {
                return ResponseEntity.ok(userService.deleteProfileImage(id, currentUser.getId()));
        }
}
